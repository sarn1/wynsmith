...
<li <?php comment_class(); ?> id="li-comment-<?php comment_ID() ?>">
  <div class="comment-author vcard">
    ...
    <div class="info">
      ...
      <small
        class="comment-date"><?php printf(__('%1$s at %2$s'), get_comment_date(), get_comment_time()) ?><?php edit_comment_link(__('(Edit)'), '  ', '') ?></small>
    </div>
  </div>
...
/*
 * get_listings ()
 *	listing_category = residential, rental, commercial
...
/*
function get_listings_detail ($permalink)
{
  global $wpdb;
  return $wpdb->get_results($wpdb->prepare(
    "
...
function print_listings ($listing_category, $listing_type = null) { ?>
  <article class="listings">
    <h2><?php echo strtoupper($listing_type)?> <?php echo strtoupper($listing_category)?> LISTINGS</h2>
    <?php
    $listings = get_listings(strtolower($listing_category), strtolower($listing_type));

    $prev_taxonomy = null;
    $prev_hood = null;
    foreach ($listings as $list) :

      //look to see if neighboorhood if different and different town or first run
      if (($list->hood <> $prev_hood || $list->taxonomy_name <> $prev_taxonomy) || is_null($prev_hood)) {
        $hood_output = '<h4>'.$list->hood.'</h4>';
        $prev_hood = $list->hood;
      } else {
        $hood_output = null;
      }

      //print taxonomy if different from previous or first run
      if ($list->taxonomy_name <> $prev_taxonomy || is_null($prev_taxonomy)): ?>
        <h3><?php echo $list->taxonomy_name?></h3>
        <?php
        $prev_taxonomy = $list->taxonomy_name;
      endif;

      if (!is_null($hood_output)) echo $hood_output;
      ?>

      <h5>
        <a href="<?php echo $list->permalink;?>"><?php echo $list->address1;?></a>
        <?php if ($list->is_sold ==1) echo "<span>(SOLD)</span>";?>
      </h5>
